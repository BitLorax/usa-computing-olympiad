...
void split(int numGroups, int start, int end, int waitTime) {
    ...
    if (cowCount - end + 1 < busCount - numGroups) {
        return;
    }
    if (numGroups != busCount) { //can split
        ...
        if (waitTime > cowTimes[end] - cowTimes[start]) {
            split(numGroups, start, end + 1, max(waitTime, cowTimes[end] - cowTimes[start]));
            return;
        }
        split(numGroups, start, end + 1, max(waitTime, cowTimes[end] - cowTimes[start]));
        if (((busCount - numGroups) * busCapacity) >= cowCount - end - 1) {
            split(numGroups + 1, end + 1, end + 1, waitTime); //split
        }
        return;
    } else {
