...
int main() {
    ...
    for (long long y = 1; y < n; ++y) {
        ...
        for (long long x = 0; x <= n; ++x) {
            if (x > 0 && !a[x - 1][y]) {
                dp[x][y][0] = sum;
                dp[x][y][0] = (dp[x][y][0] + ((!a[x - 1][y] ? 2 : 1) * (dp[x][y - 1][0] + dp[x][y - 1][1]) % MOD) % MOD) % MOD;
            } else {
                dp[x][y][0] = (dp[x][y][0] + ((dp[x][y - 1][0] + dp[x][y - 1][1]) % MOD)) % MOD;
            }
            sum = (sum + dp[x][y - 1][1]) % MOD;
            dp[x][y][0] = multi(dp[x][y][0], fastExpo(2, ctLeft(x - 2, y) + ctRight(x + 1, y)));
